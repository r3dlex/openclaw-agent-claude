defmodule Factory.MqWsClient do
  @moduledoc """
  ...
  Falls back gracefully — if WS is unavailable the HTTP-based MqClient
  continues to poll as normal.

  Configuration via environment variables:
  - IAMQ_WS_URL: WebSocket URL (default: ws://127.0.0.1:18793/ws)
  - IAMQ_AGENT_ID: This agent's ID (default: agent_claude)
  """
  use WebSockex
  require Logger
  ...
  def handle_connect(_conn, state) do
    ...
    # Defer registration to handle_info (WebSockex doesn't support reply from handle_connect)
  ...
  defp route_request(message) do
    ...
    cond do
      String.contains?(subject, "session") or String.contains?(subject, "build") ->
        Logger.info("[MQ-WS] Session/build request from #{from} — queuing")

      String.contains?(subject, "review") or String.contains?(subject, "code-review") ->
        Logger.info("[MQ-WS] Code review request from #{from}")
